...
public interface FilterRepository extends JpaRepository<Tasks,Integer> , JpaSpecificationExecutor<PatientTasksDTO> {
    ...
    @Query(value = "SELECT \n" +
                    ...
                    "WHERE \n" +
                    "t.priority IN(:priorityList) OR t.priority IS NULL\n" +
                    "AND t.taskStatus IN(:statusList) OR t.priority IS NULL\n" +
                    "AND t.dueDate BETWEEN :currentDateTime AND :plusPreset OR t.priority IS NULL")   //OR t.priority IN(:priorityList) OR t.taskStatus IN(:statusList) OR t.dueDate BETWEEN :currentDateTime AND :plusPreset
