...
public interface FilterRepository extends JpaRepository<Tasks,Integer> , JpaSpecificationExecutor<PatientTasksDTO> {
    ...
    @Query(value = "SELECT " +
                    ...
                    "WHERE " +
                    "(t.priority IN (:priorityList) OR COALESCE(:priorityList) IS NULL) " +
                    "AND (t.taskStatus IN (:statusList) OR COALESCE(:statusList) IS NULL) " +
                    "AND ((t.dueDate BETWEEN :currentDateTime AND :plusPreset) OR :currentDateTime IS NULL)")
